...
include '../src/includes/num2str.php';
...
$app->get('/api/numinwords/{number}', function(Request $request, Response $response, array $args){
	$number = $args['number'];
...
$app->get('/api/quadraticEquation', function(Request $request, Response $response, array $args){
	...
	if (isset($x1)) {
	    $result = 'Решение уравнения: корни равны  ' . $x1 . ' и ' . $x2;
	}
...
$app->get('/api/dayOfWeek', function(Request $request, Response $response, array $args){
	$allParams = $request->getQueryParams();
	$date = $allParams['date'];
